...
public static class ComputeHelper
{
    ...
    public static void Dispatch(ComputeShader cs, int numIterationsX, int numIterationsY = 1, int numIterationsZ = 1, int kernelIndex = 0)
    {
        if (cs == null)
        {
            Debug.LogWarning("Compute dispatch skipped because the compute shader is not assigned.");
            return;
        }

        numIterationsX = Mathf.Max(1, numIterationsX);
        numIterationsY = Mathf.Max(1, numIterationsY);
        numIterationsZ = Mathf.Max(1, numIterationsZ);

        Vector3Int threadGroupSizes = GetThreadGroupSizes(cs, kernelIndex);
        int groupSizeX = Mathf.Max(1, threadGroupSizes.x);
        int groupSizeY = Mathf.Max(1, threadGroupSizes.y);
        int groupSizeZ = Mathf.Max(1, threadGroupSizes.z);
